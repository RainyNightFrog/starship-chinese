/**
 * 家長自訂詞彙 — Fuse.js 模糊配對引擎
 * ─────────────────────────────────────
 * · 禁止 Math.random() 盲抽舊題庫
 * · 依家長輸入順序，逐詞對 IDIOM_EXAM_POOL 做 Fuse.js 模糊匹配（容忍 OCR 錯字）
...
export function normalizeCustomWordList(input = [], options = {}) {
  ...
  const pushWord = (raw) => {
    ...
    /** Fuse 命中池內詞、或 hint 庫詞 — 才保留；長串未知詞拒絕 */
    const fuseHit = fuzzyMatchIdiomWord(word);
    const knownInPool = Boolean(fuseHit?.item) || POOL_BY_WORD.has(word);
    if (!knownInPool && !VOCAB_HINTS[word] && !WORKSHEET_VOCAB_HINTS[word] && word.length > 4) return;
...
/**
 * Fuse.js 模糊配對核心 — 逐詞查找 IDIOM_EXAM_POOL（禁止隨機 index 盲抽）
...
export function matchCustomWordsStrict(customWordsInput = [], options = {}) {
  ...
  return words.map((userWord, index) => {
    ...
    const fuseResult = fuzzyMatchIdiomWord(canon) ?? fuzzyMatchIdiomWord(userWord);

    if (fuseResult?.item) {
      return normalizeVocabMatchItem({
        ...fuseResult.item,
        source: fuseResult.item.source ?? 'idiom_exam_pool',
        matchedFromUpload: true,
        matchedVia: fuseResult.matchedVia,
        ocrCorrectedFrom: fuseResult.matchedVia !== 'exact' ? userWord : undefined,
      }, index);
    }
...
export function resolveCustomVocabFromInput(customWordsInput = [], options = {}) {
  ...
  return {
    ...
    matchedQuestions,
    matchedCount: matchedQuestions.filter((q) => isPoolIdiomWord(q.word)).length,
